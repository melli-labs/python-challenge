...
def camelize(key: str):
    ...
    # method 1: use split, join, and capitalize
    words = key.split("_")
    for i in range(1,len(words)):
        words[0] += words[i].capitalize()
    key = words[0]
    return key
...
def handle_call_action(action: str, username: str, friendname: str = None):
    if friendname:
        return {"message": f"🤙 Calling {friendname} ..."}
    else:
...
def task3_action(request: ActionRequest):
    ...
    # handle known and unknown users
    if request.username not in friends.keys():
        ...
        friends_list = friends[request.username]
    # strip the punctuations then split it into a list-> words.
    words = re.sub(r'[^\w\s]', '', request.action).split()
    friendname, isCall = "", False
    handler = handle_unknown_action
    for word in words:
        # find user's friends name
        if word in friends_list:
            friendname = word
        # handle diff actions
        word = word.lower()
    ...
    if not handler:
        handler = handle_unknown_action
    if isCall:
        return handler(request.action, request.username, friendname)
...
async def get_current_user(token: str = Depends(oauth2_scheme)) -> User:
    ...
    if int(datetime.utcnow().timestamp()) <= payload["exp"]:  
        return User(**fake_users_db[username])
    else:
